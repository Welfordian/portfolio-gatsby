            ...
            <div className={`overflow-hidden flex flex-col md:flex-row mt-0 ml-0 md:ml-4 justify-between fixed md:mt-8 container mx-4 md:p-4 z-50 pointer-events-none`}>
                {
                    this.state.isMobile 
                    ? 
                        <div className={`${this.state.navOpen ? 'bg-white w-full' : 'w-8'} transition-all md:hidden pointer-events-auto p-2 flex items-center justify-between`} onClick={() => this.toggleNav()}>
                            {
                                this.state.navOpen
                                ?
                                    <FontAwesomeIcon
